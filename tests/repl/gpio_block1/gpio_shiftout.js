// GPIO block 1 shiftOut functional test
// Covers: shiftOut, setWatch, clearWatch, pinMode, digitalRead

var TEST_NAME = "gpio_shiftout";
var TARGET = "AUTO";
var TIMEOUT_MS = 1500;

var SHIFT_VALUE = 0xA5;
var EXPECTED_BITS = [1, 0, 1, 0, 0, 1, 0, 1];

var CFGS = {
  ESP32_V1 : {
    name : "ESP32_V1",
    boardIds : ["ESP32_IDF4", "ESP32_IDF5", "ESP32"],
    harnessName : "ESP32 DevKitC V4 / ESP32_V1 harness",
    mode : "ESP32_BASELINE_HARDWARE",
    consoleInfo : "UART0 via board USB-UART on D1/D3",
    selectorInfo : "SEL_D33=1-2 SEL_D26=1-2 SEL_D35=I2C_INT JP_UART_LOOP2=open",
    SHIFT_DATA_OUT : "D32",
    SHIFT_DATA_IN  : "D33",
    SHIFT_CLK_OUT  : "D25",
    SHIFT_CLK_IN   : "D26"
  },
  ESP32_C3 : {
    name : "ESP32_C3",
    boardIds : ["ESP32C3_IDF4", "ESP32C3_IDF5", "ESP32C3"],
    harnessName : "ESP32-C3-DevKitC-02 harness",
    mode : "C3_BASELINE_GPIO",
    consoleInfo : "UART0 via board USB-UART on D20/D21",
    selectorInfo : "SEL_D1=a2-b2 SEL_D2=a2-b2 SEL_D3=a2-b2 SEL_D4=a2-b2 J10=open SEL_D08=open SEL_D0=open",
    SHIFT_DATA_OUT : "D1",
    SHIFT_DATA_IN  : "D2",
    SHIFT_CLK_OUT  : "D3",
    SHIFT_CLK_IN   : "D4"
  }
};

function resolveCfg() {
  if (TARGET !== "AUTO") return CFGS[TARGET] || null;

  var boardId = process.env.BOARD || "";
  for (var k in CFGS) {
    var ids = CFGS[k].boardIds || [];
    if (ids.indexOf(boardId) >= 0) return CFGS[k];
  }
  return null;
}

function resolvePin(pinName) {
  try {
    return eval(pinName);
  } catch (e) {
    return null;
  }
}

function resolvePins(cfg) {
  if (!cfg) return null;

  var resolved = {
    SHIFT_DATA_OUT : resolvePin(cfg.SHIFT_DATA_OUT),
    SHIFT_DATA_IN  : resolvePin(cfg.SHIFT_DATA_IN),
    SHIFT_CLK_OUT  : resolvePin(cfg.SHIFT_CLK_OUT),
    SHIFT_CLK_IN   : resolvePin(cfg.SHIFT_CLK_IN)
  };

  if (!resolved.SHIFT_DATA_OUT || !resolved.SHIFT_DATA_IN ||
      !resolved.SHIFT_CLK_OUT || !resolved.SHIFT_CLK_IN) {
    return null;
  }

  return resolved;
}

var CFG = resolveCfg();
var PINS = resolvePins(CFG);
var done = false;
var timeoutId;
var timerIds = [];
var watchIds = [];
var checksTotal = 0;
var checksPassed = 0;
var checksFailed = 0;

function info(key, value) {
  print("INFO " + key + "=" + value);
}

function metric(key, value) {
  print("METRIC " + key + "=" + value);
}

function pass(name, extra) {
  checksTotal++;
  checksPassed++;
  print("PASS " + name + (extra ? " " + extra : ""));
}

function fail(name, extra) {
  checksTotal++;
  checksFailed++;
  print("FAIL " + name + (extra ? " " + extra : ""));
}

function expectEq(name, actual, expected) {
  if (actual === expected) {
    pass(name, "got=" + actual + " expected=" + expected);
  } else {
    fail(name, "got=" + actual + " expected=" + expected);
  }
}

function expectJsonEq(name, actual, expected) {
  var actualText = JSON.stringify(actual);
  var expectedText = JSON.stringify(expected);
  if (actualText === expectedText) {
    pass(name, "got=" + actualText + " expected=" + expectedText);
  } else {
    fail(name, "got=" + actualText + " expected=" + expectedText);
  }
}

function safeCall(fn) {
  try {
    fn();
  } catch (e) {
    // Cleanup should not block completion reporting.
  }
}

function schedule(delayMs, fn) {
  var id = setTimeout(fn, delayMs);
  timerIds.push(id);
  return id;
}

function addWatch(pin, options, fn) {
  var id = setWatch(fn, pin, options);
  watchIds.push(id);
  return id;
}

function clearAllWatches() {
  while (watchIds.length) {
    var id = watchIds.pop();
    safeCall(function() { clearWatch(id); });
  }
}

function clearAllTimers() {
  while (timerIds.length) {
    var id = timerIds.pop();
    safeCall(function() { clearTimeout(id); });
  }
}

function cleanup() {
  clearAllTimers();
  clearAllWatches();

  if (!PINS) return;

  safeCall(function() { digitalWrite(PINS.SHIFT_DATA_OUT, 0); });
  safeCall(function() { digitalWrite(PINS.SHIFT_CLK_OUT, 0); });

  safeCall(function() { pinMode(PINS.SHIFT_DATA_OUT, "input"); });
  safeCall(function() { pinMode(PINS.SHIFT_DATA_IN, "input"); });
  safeCall(function() { pinMode(PINS.SHIFT_CLK_OUT, "input"); });
  safeCall(function() { pinMode(PINS.SHIFT_CLK_IN, "input"); });
}

function finish() {
  if (done) return;
  done = true;

  if (timeoutId) clearTimeout(timeoutId);

  cleanup();

  metric("checks_total", checksTotal);
  metric("checks_passed", checksPassed);
  metric("checks_failed", checksFailed);

  echo(true);
  print("DONE=" + TEST_NAME);
}

function runShiftOutPhase() {
  var bitsSeen = [];
  var clockCount = 0;
  var watchId;

  pinMode(PINS.SHIFT_DATA_OUT, "output");
  pinMode(PINS.SHIFT_DATA_IN, "input");
  pinMode(PINS.SHIFT_CLK_OUT, "output");
  pinMode(PINS.SHIFT_CLK_IN, "input");

  digitalWrite(PINS.SHIFT_DATA_OUT, 0);
  digitalWrite(PINS.SHIFT_CLK_OUT, 0);

  watchId = addWatch(
    PINS.SHIFT_CLK_IN,
    {repeat:true, edge:"rising", debounce:0, data:PINS.SHIFT_DATA_IN},
    function(e) {
      clockCount++;
      bitsSeen.push(e.data ? 1 : 0);
    }
  );

  schedule(20, function() {
    shiftOut(PINS.SHIFT_DATA_OUT, {clk:PINS.SHIFT_CLK_OUT, clkPol:1, repeat:8}, [SHIFT_VALUE]);
  });

  schedule(200, function() {
    clearWatch(watchId);
    metric("gpio_shiftout_clock_edges", clockCount);
    metric("gpio_shiftout_bits_seen", JSON.stringify(bitsSeen));
    expectEq("gpio_shiftout_clock_count", clockCount, 8);
    expectJsonEq("gpio_shiftout_bits", bitsSeen, EXPECTED_BITS);
    expectEq("gpio_shiftout_data_final", digitalRead(PINS.SHIFT_DATA_IN), EXPECTED_BITS[EXPECTED_BITS.length - 1]);
    finish();
  });
}

function run() {
  var boardId = process.env.BOARD || "UNKNOWN";

  echo(false);
  print("TEST=" + TEST_NAME);
  print("TARGET=" + (CFG ? CFG.name : "UNRESOLVED"));
  info("board", boardId);
  info("api", "shiftOut,setWatch,clearWatch,pinMode,digitalRead");

  if (!CFG) {
    fail("config_resolve", "target=" + TARGET + " board=" + boardId);
    finish();
    return;
  }

  if (!PINS) {
    fail("pin_resolve", "target=" + CFG.name);
    finish();
    return;
  }

  info("harness", CFG.harnessName);
  info("mode", CFG.mode);
  info("console", CFG.consoleInfo);
  info("selectors", CFG.selectorInfo);

  timeoutId = setTimeout(function() {
    fail("timeout", "ms=" + TIMEOUT_MS);
    finish();
  }, TIMEOUT_MS);

  runShiftOutPhase();
}

run();
