// UART block 7 repeated setup/unsetup soak functional test
// Covers: Serial.setup, Serial.write, Serial.read, Serial.unsetup repeated lifecycle

echo(false);

var TEST_NAME = "uart_repeated_setup_soak";
var TARGET = "AUTO";
var TIMEOUT_MS = 9000;
var SETTLE_MS = 180;
var INTER_ITER_MS = 80;
var ITERATIONS = 10;
...
function runIteration(index) {
  if (index >= ITERATIONS) { finish(); return; }
  var baud = (index % 2) ? 57600 : 115200;
  var aToB = (index % 2) === 0;
  var sender = aToB ? PORTS.SERIAL_A : PORTS.SERIAL_B;
  var receiver = aToB ? PORTS.SERIAL_B : PORTS.SERIAL_A;
  var label = aToB ? "s2_to_s3" : "s3_to_s2";
  var payload = "SOAK_" + index + "_" + label;
  var checkName = "uart_soak_iter_" + index;
  ...
  schedule(INTER_ITER_MS, function() {
    ...
    schedule(SETTLE_MS, function() {
      ...
      metric(checkName + "_direction", label);
      expectEq(checkName + "_rx", got, payload);
      schedule(INTER_ITER_MS, function() { runIteration(index + 1); });
    });
  });
}
...
function run() {
  ...
  info("board", boardId);
  info("api", "Serial.setup,Serial.write,Serial.read,Serial.unsetup,repeated_lifecycle");
