// UART block 7 listener variant functional test
// Covers: Serial.setup, Serial.on, addListener, prependListener, removeListener, removeAllListeners

echo(false);

var TEST_NAME = "uart_listener_variants";
var TARGET = "AUTO";
var TIMEOUT_MS = 5000;
var BAUD = 115200;
var SETTLE_MS = 180;
...
function run() {
  ...
  var sender = PORTS && PORTS.SERIAL_A;
  var receiver = PORTS && PORTS.SERIAL_B;
  var order = "";
  var lateText = "";
  function first(d) { order += "first:" + d + "|"; }
  function added(d) { order += "add:" + d + "|"; }
  function prepended(d) { order += "prep:" + d + "|"; }

  print("TEST=" + TEST_NAME);
  print("TARGET=" + (CFG ? CFG.name : "UNRESOLVED"));
  info("board", boardId);
  info("api", "Serial.setup,on,addListener,prependListener,removeListener,removeAllListeners");
  ...
  drain(receiver);
  receiver.port.on("data", first);
  receiver.port.addListener("data", added);
  receiver.port.prependListener("data", prepended);
  sender.port.write("ORDER");
  schedule(SETTLE_MS, function() {
    metric("uart_listener_order", JSON.stringify(order));
    expectEq("uart_listener_prepend_order", order, "prep:ORDER|first:ORDER|add:ORDER|");

    order = "";
    receiver.port.removeListener("data", first);
    sender.port.write("REMOVE");
    schedule(SETTLE_MS, function() {
      metric("uart_listener_after_remove", JSON.stringify(order));
      expectEq("uart_listener_remove_specific", order, "prep:REMOVE|add:REMOVE|");

      receiver.port.removeAllListeners("data");
      sender.port.write("LATE");
      schedule(SETTLE_MS, function() {
        receiver.port.addListener("data", function(d) { lateText += d; });
        schedule(SETTLE_MS, function() {
          metric("uart_listener_late_text", JSON.stringify(lateText));
          expectEq("uart_listener_buffer_delivered_on_attach", lateText, "LATE");
          finish();
        });
      });
    });
