// UART block 7 pipe functional test
// Covers: Serial.setup, Serial.write, Serial.pipe, Serial.read, Serial.unsetup

echo(false);

var TEST_NAME = "uart_pipe_to_sink";
var TARGET = "AUTO";
var TIMEOUT_MS = 5000;
var BAUD = 115200;
var SETTLE_MS = 180;
var PIPE_SETTLE_MS = 260;
...
function removeListenersAndUnsetup(portInfo) {
  ...
  safeCall(function() { portInfo.port.removeAllListeners("close"); });
...
function run() {
  ...
  var payload = "PIPE_PHYSICAL_UART_DATA";
  var sink = {
    chunks : 0,
    data : "",
    write : function(d) {
  ...
  info("board", boardId);
  info("api", "Serial.setup,Serial.write,Serial.pipe,Serial.read,Serial.unsetup");
  ...
  PORTS.SERIAL_A.port.write(payload);
  schedule(SETTLE_MS, function() {
    PORTS.SERIAL_B.port.pipe(sink, {
      chunkSize : 5,
      end : false,
      complete : function() { pipeComplete = true; }
    });
    schedule(PIPE_SETTLE_MS, function() {
      metric("uart_pipe_sink_chunks", sink.chunks);
      metric("uart_pipe_sink_data", JSON.stringify(sink.data));
      metric("uart_pipe_complete", pipeComplete);
      expectTrue("uart_pipe_sink_chunked", sink.chunks >= 2, "chunks=" + sink.chunks);
      expectEq("uart_pipe_sink_data", sink.data, payload);
      expectEq("uart_pipe_open_stream_not_complete", pipeComplete, false);
      finish();
    });
  });
}
