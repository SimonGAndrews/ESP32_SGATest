...
var TEST_NAME = "uart_rx_burst_128_s2_to_s3";
var TARGET = "AUTO";
var TIMEOUT_MS = 3000;
var BAUD = 115200;
var CASE_LENGTHS = [128];
var DIRECTION = {
  senderKey : "SERIAL_A",
  receiverKey : "SERIAL_B",
  label : "s2_to_s3"
};
...
function resolvePin(pinName) {
  try { return eval(pinName); } catch (e) { return null; }
}

function resolvePort(portName) {
  try { return eval(portName); } catch (e) { return null; }
}
...
function resolvePins(cfg) {
  ...
  if (!resolved.SERIAL_A.tx || !resolved.SERIAL_A.rx || !resolved.SERIAL_A.port ||
      !resolved.SERIAL_B.tx || !resolved.SERIAL_B.rx || !resolved.SERIAL_B.port) {
    return null;
  }
  return resolved;
}
...
function run() {
  ...
  runBurstCase(0, sender, receiver, DIRECTION.label, function() { finish(); });
}
run();
