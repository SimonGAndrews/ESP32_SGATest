#!/usr/bin/env python3
"""Run analog feedback checks against an ESP32_V1 Espruino REPL."""
...
digitalWrite(D27, 0);
setTimeout(function() {
  emit("ADC_LOW", analogRead(D34));
  digitalWrite(D27, 1);
  setTimeout(function() {
    emit("ADC_HIGH", analogRead(D34));
    analogWrite(D27, 0.25);
    setTimeout(function() {
      emit("ADC_PWM_25", analogRead(D34));
      analogWrite(D27, 0.50);
      setTimeout(function() {
        emit("ADC_PWM_50", analogRead(D34));
        analogWrite(D27, 0.75);
        setTimeout(function() {
          emit("ADC_PWM_75", analogRead(D34));
          finish();
        }, 120);
      }, 120);
    }, 120);
